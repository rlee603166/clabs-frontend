    ...
    return (
      <div className="flex justify-end">
        <div className="bg-blue-500/80 text-white rounded-2xl p-3 max-w-xs backdrop-blur-sm border border-blue-500/30">
          ...
          <div className="text-xs text-blue-100 mt-1">{message.timestamp.toLocaleTimeString()}</div>
        </div>
      </div>
    )
  }
          ...
          {message.latency && (
            <div className="flex items-center space-x-2 mb-3">
              ...
              {Object.entries(message.latency).map(([store, time]) => (
                <Badge key={store} className="text-xs border-gray-400/30 text-gray-700 bg-gray-400/10 backdrop-blur-sm">
            ...
            <Collapsible open={showQuery} onOpenChange={setShowQuery}>
              <CollapsibleTrigger asChild>
                <Button
                  variant="ghost"
                  size="sm"
                  className="text-gray-600 hover:text-gray-800 hover:bg-white/20 p-0 h-auto"
                >
                  ...
                  <div className="space-y-2">
                    {message.functions.map((func, i) => (
                      <div key={i} className="flex items-center justify-between text-xs">
                        <code className="text-blue-700 font-mono">{func}()</code>
                        <Button
                          variant="ghost"
                          size="sm"
                          className="h-auto p-1 text-gray-600 hover:text-gray-800 hover:bg-white/20"
                        >
